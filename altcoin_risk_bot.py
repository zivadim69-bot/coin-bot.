...
def compute_contract_verdict(security):
    ...
    if flags:
        meaning = "обнаружены красные флаги по базовым метрикам: " + "; ".join(flags)
        action = "сводка ограничена этими признаками; решение о сделке требует отдельной проверки"
    elif unknowns:
        meaning = "по проверенным метрикам красных флагов нет, но " + ", ".join(unknowns)
        action = "вывод неполный: недостающие данные требуют отдельной проверки"
    else:
        meaning = "явных красных флагов не найдено по базовым метрикам"
        action = "сводка описательная и не является финансовой рекомендацией"
