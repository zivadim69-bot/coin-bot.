...
def get_dex_magnets(chain, pair_address, current_price):
    """Уровни-магниты через GeckoTerminal (свечи по адресу пула - работает для любой сети)."""
    if not chain or not pair_address or not current_price:
        return None
    try:
        url = f"https://api.geckoterminal.com/api/v2/networks/{chain}/pools/{pair_address}/ohlcv/day"
        params = {"aggregate": 1, "limit": 7}
        r = requests.get(url, params=params, timeout=15)
        r.raise_for_status()
        rows = r.json().get("data", {}).get("attributes", {}).get("ohlcv_list", [])
        if not rows:
            return None
        # каждая строка: [timestamp, open, high, low, close, volume]
...
def main():
    ...
    if dex and dex.get("pair_address"):
        magnets = get_dex_magnets(dex["chain"], dex["pair_address"], price_for_magnets)
    if not magnets:
